...
function AskQuestions({isNight}) {
    ...
    const fetchData = async () => {
      try {
        ...
        const response = await fetch(
          "http://localhost:3000/Questions/userPlan",
          config
        );
  ...
  async function getLastQuestionTime() {
    try {
      if (shareConfig) {
        const response = await fetch(
          "http://localhost:3000/Questions/gettingTime",
          shareConfig
        );
        if (response.ok) {
  ...
  const setLastQuestionTime = async (updatedTime) => {
    try {
      const response = await axios.post(
        "http://localhost:3000/Questions/updateQuestionTime",
        { updatedTime },
        shareConfig
      );
    } catch (error) {
  ...
  return (
    <>
      <div className="ask_public_questions_page" style={{backgroundColor: isNight?"#060A13":""}}>
        <div className="ask_public_question">
          <h1>Ask a public Question [{plan}]</h1>
          <div className="ask_question_form" style={{backgroundColor: isNight?"#060A13":""}}>
